...
function createSupabaseMock(options: {
  user?: { id: string } | null
  job?: { id: string } | null
  duplicate?: { id: string; status?: string; applied_at: string | null } | null
...
describe('POST /api/extension/track-application', () => {
  ...
  it('returns the existing application as an idempotent replay for duplicate apply URLs', async () => {
    createSupabaseMock({
      duplicate: { id: 'existing-app-1', status: 'saved', applied_at: null },
      upsertResult: { id: 'existing-app-1', status: 'saved' },
    })
    ...
    await expect(response.json()).resolves.toEqual(
      expect.objectContaining({
        ...
        status: 'saved',
        idempotent: true,
        idempotencyKey: 'apply_url:https://jobs.example.com/apply',
      })
    )
  })
})
