...
it('redirects to /login with a short-lived exchange code for extension sign-in', async () => {
  const req = new NextRequest('http://localhost/api/auth/callback?code=test_code&source=extension')
  const response = await GET(req)

  expect(response.status).toBe(302)
  expect(response.headers.get('location')).toContain('/login?')
  expect(response.headers.get('location')).toContain('exchange_code=exchange-code')
